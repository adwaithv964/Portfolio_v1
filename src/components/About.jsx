...
// Safe renderer — only renders <strong> and <em>, all else is plain text
function SafeHtml({ html, className }) {
  if (!html) return null;
  const parts = [];
  let last = 0;
  const re = /<(strong|em)>(.*?)<\/(strong|em)>/gi;
  let m;
  while ((m = re.exec(html)) !== null) {
    if (m.index > last) parts.push(<span key={last}>{html.slice(last, m.index)}</span>);
    const Tag = m[1].toLowerCase();
    parts.push(<Tag key={m.index} className={Tag === 'strong' ? 'text-white font-bold' : 'italic'}>{m[2]}</Tag>);
...
const About = () => {
  ...
  return (
    <section
      id="about"
      ref={sectionRef}
      className="relative w-full min-h-screen bg-[#050505] text-white py-16 md:py-32 px-5 md:px-12 flex flex-col justify-center select-none overflow-hidden"
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full space-y-10 md:space-y-16">
        ...
        <div className="flex flex-col items-start space-y-4">
          ...
          <h2 className="text-3xl md:text-6xl font-black tracking-tighter text-white">
            EPISODE SYNOPSIS <br />
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div
            ref={addToRefs}
            className="md:col-span-7 p-6 md:p-12 bg-[#141414]/90 backdrop-blur-2xl border border-white/10 rounded-2xl md:rounded-[2.5rem] shadow-2xl flex flex-col justify-between relative group hover:border-red-600/60 transition-all duration-500 overflow-hidden"
          >
            ...
            <div className="space-y-5 relative z-10">
              ...
              <p className="text-base md:text-xl font-medium text-white/90 leading-relaxed">
                <SafeHtml html={a.bio} />
              </p>
            ...
            <div className="pt-8 flex flex-wrap gap-2 relative z-10">
              {(a.tags || []).map((tag, i) => (
                <span key={i} className="px-3.5 py-1.5 rounded bg-white/5 border border-white/10 text-xs font-mono text-white/80">{tag}</span>
              ))}
            </div>
          </div>
          ...
          <div
            ref={addToRefs}
            className="md:col-span-5 p-6 md:p-12 bg-[#141414]/90 backdrop-blur-2xl border border-white/10 rounded-2xl md:rounded-[2.5rem] shadow-2xl flex flex-col justify-between relative group hover:border-red-600/60 transition-all duration-500 overflow-hidden"
          >
            {/* Real-time mouse hover spotlight highlight */}
            <div 
              className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            ...
            <div className="space-y-5 relative z-10">
              <h3 className="text-xs font-mono uppercase tracking-widest text-red-500 font-bold">{a.milestonesHeader}</h3>
